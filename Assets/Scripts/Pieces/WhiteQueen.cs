        ...
        public static ulong ValidMoves(BoardState state, ulong piecePosition)
        {
            ...
            while (!upStopped || !downStopped || !leftStopped || !rightStopped || !upLeftStopped || !upRightStopped || !downLeftStopped || !downRightStopped)
            {
                if (!upStopped)
                {
                    ...
                    if ((up & whitePieces) > 0) upStopped = true;
                    else if ((up & blackPieces) > 0 || (up & BoardPositionalInfo.Row8) > 0)
                    {
                        validMoves |= up;
                        upStopped = true;
                    }
                ...
                if (!downStopped)
                {
                    ...
                    if ((down & whitePieces) > 0) downStopped = true;
                    else if ((down & blackPieces) > 0 || (down & BoardPositionalInfo.Row1) > 0)
                    {
                        validMoves |= down;
                        downStopped = true;
                    }
                ...
                if (!leftStopped)
                {
                    ...
                    if ((left & whitePieces) > 0) leftStopped = true;
                    else if ((left & blackPieces) > 0 || (left & BoardPositionalInfo.ColA) > 0)
                    {
                        validMoves |= left;
                        leftStopped = true;
                    }
                ...
                if (!rightStopped)
                {
                    ...
                    if ((right & whitePieces) > 0) rightStopped = true;
                    else if ((right & blackPieces) > 0 || (right & BoardPositionalInfo.ColH) > 0)
                    {
                        validMoves |= right;
                        rightStopped = true;
                    }
                ...
                if (!upLeftStopped)
                {
                    ...
                    if ((upLeft & whitePieces) > 0) upLeftStopped = true;
                    else if ((upLeft & blackPieces) > 0 || (upLeft & BoardPositionalInfo.Row8) > 0 || (upLeft & BoardPositionalInfo.ColA) > 0)
                    {
                        validMoves |= upLeft;
                        upLeftStopped = true;
                    }
                ...
                if (!upRightStopped)
                {
                    ...
                    if ((upRight & whitePieces) > 0) upRightStopped = true;
                    else if ((upRight & blackPieces) > 0 || (upRight & BoardPositionalInfo.Row8) > 0 || (upRight & BoardPositionalInfo.ColH) > 0)
                    {
                        validMoves |= upRight;
                        upRightStopped = true;
                    }
                ...
                if (!downLeftStopped)
                {
                    ...
                    if ((downLeft & whitePieces) > 0) downLeftStopped = true;
                    else if ((downLeft & blackPieces) > 0 || (downLeft & BoardPositionalInfo.Row1) > 0 || (downLeft & BoardPositionalInfo.ColA) > 0)
                    {
                        validMoves |= downLeft;
                ...
                if (!downRightStopped)
                {
                    ...
                    if ((downRight & whitePieces) > 0) downRightStopped = true;
                    else if ((downRight & blackPieces) > 0 || (downRight & BoardPositionalInfo.Row1) > 0 || (downRight & BoardPositionalInfo.ColH) > 0)
                    {
